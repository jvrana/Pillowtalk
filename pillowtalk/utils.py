    ...
    def wrapped(*args, **kwargs):
        ...
        if len(args) > len(fields):
            raise TypeError("Expected {0} arguments but got {1} for {2}.__init__".format(
                    len(fields), len(args), classname))
        for key, val in kwargs.items():
            if key in field_dict:
                raise PillowtalkInitializerError("{0} got multiple values for {1}".format(classname, key))
            else:
                field_dict[key] = val
        for field in fields:
            if field not in field_dict.keys():
                raise PillowtalkInitializerError("{0} is missing argument {1}".format(classname, field))
